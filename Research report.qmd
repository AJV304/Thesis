...
Y = \beta_0 + \beta_1X + \epsilon,
$$ 
where $Y$ is the dependent variable, $\beta_0$ the intercept, $\beta_1$ the main effect, and $\epsilon$ the random error. The population parameter values are presented in Table @tbl-DGM, together with the parameter values for the deviation conditions. The complete data-generating model is defined as
\begin{align*}
Y =&  \beta_0 + \beta_1X + \beta_2Z_1 + \beta_3Z_2 + \epsilon,\\
Y_a =&  \beta_0 + \beta_1X + \beta_2Z_1 + \beta_3Z_2 + \epsilon,
\end{align*}
where $Z_1$ and $Z_2$ represent a continuous and categorical covariate (respectively) and $Y_a$ represents an alternative outcome variable.

Data is generated under two scenarios: 'X has an effect on Y' and 'X has no effect on Y'. Both scenarios generate data by producing parametric draws from a multivariate normal distribution for $X$, $Z_1$ and $Z_2$. The difference in data generation between the two scenarios is in the generation of $Y$ from the predictor space. In the 'no effect' scenario, $\beta_1$ is 0, and in the 'effect' scenario, $\beta_1$ will be 0.3.

In order to asses differences between outlier identification methods, the data need to include outliers. In order to simulate outliers, 5% of the data is simulated with an increased or decreased value for $\epsilon$. The parameter values for the data generating mechanism are specified in @tbl-DGM.
