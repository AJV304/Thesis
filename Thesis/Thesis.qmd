...
|                                          |                      | -30                                |
+------------------------------------------+----------------------+------------------------------------+
| Outlier exclusion criteria               | Exclusion based on a *z*-scores of 3 | Exclusion based on *z*-scores of 2      |
+------------------------------------------+----------------------+------------------------------------+
|                                          |                      | Exclusion based on Cook's distance larger than 1 |
+------------------------------------------+----------------------+------------------------------------+
| Statistical methods: covariates       | No covariates        | Continuous covariate               |
|                                          |                      |                                    |
|                                          |                      | Dichotomous covariate              |
+------------------------------------------+----------------------+------------------------------------+
| Statistical methods: outcome switching | Y                    | Ya (\rho(Y, Y_a) = 0.6)                                |
+------------------------------------------+----------------------+------------------------------------+
...
With an MCSE of 0.01, power rates are assessed at a precision interval of \[79, 81\] around the nominal power level of 80%. Power levels outside of this range can be stated to have increased or deflated power levels that are not due to simulation noise.
...
An R package was created for this project under the name `thepack` in which all functions necessary to simulate the data can be found. The package is publicly available at [githublink]. In order to advance reproducibility, all other files related to this project are publicly available at [github link]. This includes data, code scripts and output files. I made use of `renv` to create a reproducible environment in which package versions and R version are recorded. Instructions on how to reproduce the findings can be found under the README file on GitHub.
...
The primary research aim of this study is to examine the effects of deviations from preregistration on type I and type II errors. In this section the type I and II errors of each deviation condition are compared to those of the baseline condition **OR NOMINAL?**
