...
Preregistration was proposed as a way to limit *p*-hacking and other opportunistic researcher choices. A preregistration is a document in which decisions concerning data collection and analysis are recorded. The document is then uploaded to a public repository, with a time stamp of when it was made public. Peer reviewers, and others, have access to the preregistration and can evaluate it before the start of the study or compare it to the final research paper. Preregistration aims to increase transparency about research choices and has become more popular, with the number of preregistrations increasing annually [@ferguson2023; @lindsay2018].
...
In the simulated model, the estimand is $\beta_1,$ which represents the effect of the independent variable $X$ on dependent variable $Y$. The coefficient is estimated using the `lm()` function from the `stats` package in `R` [@rcoreteam2024], under the baseline and deviation conditions.
...
Data was generated by randomly drawing samples from the defined population. In case of the baseline condition, samples of size $\it{n} = 200$ were drawn, outliers were excluded based on an absolute *z*-score of greater than 3 and a simple linear regression was run from which the $\beta_1$ regression coefficient , the *p*-value and the confidence interval were recorded. For the nine deviation conditions, the specific deviation was applied to the generated data (i.e. either the sample size, outlier exclusion criterion, or statistical model was changed), and the same outcomes were recorded as in the baseline condition. This process was repeated across 1,600 repetitions. Within each repetition, all ten conditions (the baseline and nine deviation conditions) were evaluated on the same generated dataset, and outcomes were recorded for each.

To investigate the effect of different deviations, I defined nine deviations conditions and one control condition (@tbl-cond). Each condition is evaluated under the effect scenario, in which power is examined, and no-effect scenario, in which the type I error rate is examined.

To investigate the effect of deviating by default, the type I error rate and power were compared between the nominal levels and each deviation condition. In each repetition, the $\beta_1$ regression coefficient, the *p*-value and the confidence interval were recorded for each condition.

To answer the second research question, the effect of deviating opportunistically was examined. This was done by creating an "Opportunistic condition", using the same data set as for the first research question. For each repetition, the *p-*value of the baseline condition was assessed. When it was significant, the baseline condition was recorded for that repetition. If it was non-significant, the deviation conditions of that repetition were checked (@fig-opp). If any of the deviation conditions were significant, one was chosen at random and recorded for that repetition. As a result, for each repetition, a significant result was recorded if it was available among any of the conditions. This provided an 'optimized' condition, with the maximum number of significant results possible. The error rates were then compared between the baseline condition and the opportunistic condition.
