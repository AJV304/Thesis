...
Data is generaterd under two scenarios: "X has an effect on Y" and "X has no effect on Y". Within these scenarios each simulated data set is examined under different conditions. The conditions include a baseline condition, with no deviations, and one condition for each possible deviation (@tbl-cond)
