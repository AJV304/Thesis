...
In order to assess differences between outlier exclusion criteria, the data needs to include outliers. These are simulated by generating 5% of the data with increased or decreased mean values used to generate $\epsilon$.
...
The results for the primary research question show an effect of deviating on power in each of the three deviation domains in the effect scenario (@fig-rq1.y).

Within the sample size domain, small changes in sample size show no effect on power (79.9% and 80.8%) whereas larger changes in sample size did appear impactful. Commonly known, increasing sample size can increase the power of a study \[source\]. This effect is also observed in the results of this study. A 15% decrease in sample size resulted in a power decrease of `r (rq1.yes.plot[rq1.yes.plot$conditions == "Baseline", "n.sig.perc"]-rq1.yes.plot[rq1.yes.plot$conditions == "Sample size (195)", "n.sig.perc"])*100` %. This means that deviating in the form of collecting fewer than planned participants, is likely to lead to an underpowered study. When deviating with large sample size *increases*, studies are likely to increase in power. Results showed an increase in power of `r (rq1.yes.plot[rq1.yes.plot$conditions == "Sample size (230)", "n.sig.perc"]-rq1.yes.plot[rq1.yes.plot$conditions == "Baseline", "n.sig.perc"])*100`% in the 15% increase condition as compared to the baseline condition, resulting in a power level of `r rq1.yes.plot[rq1.yes.plot$conditions == "Sample size (230)", "n.sig.perc"]*100`%.

Deviations in the method of excluding outliers also impacts power levels. When using a strict *z*-score (exclusion based on 2 s.d. instead of 3 s.d.) power increases to `r rq1.yes.plot[rq1.yes.plot$conditions == "Strict z-score", "n.sig.perc"]*100` %. When using Cook's distance instead of *z*-scores, the power level drops to `r rq1.yes.plot[rq1.yes.plot$conditions == "Cook's distance", "n.sig.perc"]*100` %.

In the statistical model domain, effects are only observed in the alternative outcome condition, and not in the two covariate conditions. Deviating by using an alternative, correlated outcome variable decreases the power by `r (rq1.yes.plot[rq1.yes.plot$conditions == "Baseline", "n.sig.perc"]-rq1.yes.plot[rq1.yes.plot$conditions == "Alternative outcome", "n.sig.perc"])*100` %, resulting in a power level of `r rq1.yes.plot[rq1.yes.plot$conditions == "Alternative outcome", "n.sig.perc"]*100`%.
...
```{r}
#| label: Research question 2, effect scenario

source("../Scripts/02_results/RQ2_effect.R")
```

```{r}
#| label: Research question 2, no-effect scenario

source("../Scripts/02_results/RQ2_noeffect.R")
```

### Choosing to deviate

Choosing to deviate when the baseline condition is insignificant leads to an increased power of `r (rq2.yes.nsig.perc*100)`% in the effect scenario. In the no-effect scenario, the type I error rate increases to `r (rq2.no.perc*100)`%. In the effect scenario, `r rq2.yes %>% filter(n.sig == "NA") %>% nrow()` iterations showed a significant baseline. Of the remaining `r 1600 - rq2.yes %>% filter(n.sig == "NA") %>% nrow()` iterations, between zero and seven deviations were significant (@tbl-rq2.yes). In the no-effect scenario, the majority of the iterations showed an insignificant baseline as well as insignificant deviations. In the `r (rq2.no.perc*100)`% that did have significant findings, between one and six deviations had significant *p*-values (@tbl-rq2.no).

```{r}
#| label: tbl-rq2.yes
#| tbl-cap: "Distribution of the number of significant deviations"

print(rq2.yes.table, quote = FALSE, row.names = FALSE)
