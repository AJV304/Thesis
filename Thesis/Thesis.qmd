...
To answer the main research question, the type I and type II error rates are compared between the baseline result and each deviation result. All ten conditions, one baseline and nine deviation conditions, are executed using the same data set. This is done iteratively, where the $\beta_1$ regression coefficient, the p-value and the confidence interval are recorded for each condition.

To answer the second research question, the effect of choosing whether or not to deviate is examined. This is done using the same data set as in the first section. For each iteration, the baseline *p-*value is assessed. When it is significant, the baseline condition is recorded for that iteration, if it is insignificant, the deviations conditions of that iteration are checked. If any of the deviations conditions are significant, one is chosen at random and recorded for that iteration. This provides an 'optimized' data set, with the largest number of significant iterations possible in the data set. The error rates will then be compared between the baseline results and the 'optimized' result. An overview can be found in @fig-phases.
...
Type II error refers to a false negative, or failing to reject the null-hypothesis when it is false. In this study that would mean failing to detect an effect in the effect scenario. Type II error is commonly assessed in the form of power. Power is $1-\beta$, where $\beta$ is the type II error. The nominal type II error rate of .20 results in a generally accepted power level of 80%. In this study, type II error will be expressed in the form of power rates and consequently, power rates below 80% are considered deflated power.

In order to account for the uncertainty introduced by simulationThe number of iterations is based on the desired precision of the project. A Monte-Carlo standard error (MCSE) of 1% is deemed appropriate. According to the formula by @morris2019a,
...
With an MCSE of 1%, power rates are assessed at a precision interval of \[79, 81\] around the nominal power level of 80%. Power levels outside of this range can be stated to have increased or deflated power levels that are not due to simulation noise.
...
In order to advance reproducibility, all files related to this project are publicly available at [github link]. This includes data, code scripts and output files. I made use of `renv` to create a reproducible environment in which package versions and r version are recorded. Instructions on how to reproduce the findings can be found under the README file on github.
During this project an R package was created under the name `thepack` to store the most important functions. The package is publicly available at [githublink]. 

Ethics approval was obtained on October 7th, 2025 from the University of Utrecht under case-number #25-1980. 
...
```
The results for the primary research question show an effect of deviating on power in each of the three deviation domains (@fig-rq1.y).

Within the sample size domain, small changes in sample size show no effect on power whereas larger changes in sample size are impactful. It is an established fact that increasing sample size can increase the power of a study \[source\]. This effect is also observed in the results of this study. A 15% decrease in sample size resulted in a deflated power level of `r rq1.yes.plot[rq1.yes.plot$conditions == "Sample size (170)", "n.sig.perc"]*100` %. This means that deviating in the form of collecting fewer participants than planned, is likely to lead to an underpowered study. When deviating by large sample size *increases*, the results fail to show a negative impact. The power level increases to `r rq1.yes.plot[rq1.yes.plot$conditions == "Sample size (230)", "n.sig.perc"]*100`% in the 15% increase condition.

Deviations in the Outlier exclusion criteria domain also impact power levels. When using a strict *z*-score (exclusion based on 2 s.d. instead of 3 s.d.) power increases to `r rq1.yes.plot[rq1.yes.plot$conditions == "Strict z-score", "n.sig.perc"]*100` %. When using Cook's distance instead of *z*-scores, the power level drops to `r rq1.yes.plot[rq1.yes.plot$conditions == "Cook's distance", "n.sig.perc"]*100` %.

In the Statistical model domain, deflated power is only observed in the alternative outcome condition. Deviating by using an alternative, correlated outcome variable results in a power level of `r rq1.yes.plot[rq1.yes.plot$conditions == "Alternative outcome", "n.sig.perc"]*100`%. This is incredibly low when compared to the nominal power level of 80%. Both the continuous and dichotomous covariate conditions fail to show an effect on power.

```{r}
#| label: fig-rq1.y
#| fig-width: 8
#| fig-height: 5
#| fig-cap: "Power level for Baseline and Deviation Conditions in the Effect scenario"
#| echo: false
...
This simulation study demonstrates that type I and type II error are both affected by deviations from a preregistration. 

Power can be affected both positively and negatively, depending on the type of deviation. Collecting much fewer participants, using Cook's distance as outlier criterium and switching to an alternative outcome all deflate power levels. Power is most impacted by switching outcomes with a steep drop off from the nominal 80% to `r rq1.yes.plot[rq1.yes.plot$conditions == "Alternative outcome", "n.sig.perc"]*100`% power. Small changes in sample size and the addition of a covariate show little effect, whereas collecting many more participants than planned and using a *z*-score of two standard deviations show slight increases in power. 

Only two deviation conditions showed effects on the type I error. Small increases in sample size resulted in a small inflation of type I error and using two standard deviations as outlier criterium showed a slightly larger inflation. For all other conditions, effects on type I error were not strong enough to suggest an effect outside of simulation noise. This shows that there is little risk of inflating the type I error when being forced to deviate from a preregistration except for using a stricter *z*-score as outlier criterium. 

The small inflation when slightly increasing the sample size is a borderline case. Using the MCSE of 0.055 which was preregistered for this study means that it falls outside of the precision range, outside of which I stated that effects were unlikely to be due to simulation noise. Using the empirical MCSE of this condition however shows that the effect might still be due to noise. With a type I error of 0.055625, the empirical MCSE becomes 0.00573. This means that type I error rates higher than 0.055729 would be considered inflated, which this condition just remains beneath. When theoretically considering this effect, it is more likely that this effect is in fact due to simulation error rather than an actual effect. If this was an effect of the deviation, it would be expected to see an opposite effect when decreasing the sample size slightly as well as a stronger effect when increasing the sample size more extremely. Neither of these effects can be observed in the other sample size conditions, thus making it more likely that the observed effect is indeed due to simulation noise. (In light of this, all other conditions were also evaluated using the empirical MCSE. The slight increase in sample size remained the only condition in which this changed the conclusion.)
From these results 
