...
df_outlier <- df %>% mutate(y_no = 
                              replace(x      = y_no, 
                                      list   = repla, 
                                      values = ifelse(runif(length(repla)) < 0.5, sample(y_no_outlier_high, 0.05*n), sample(y_no_outlier_low, 0.05*n))))
...
hist(df$y_no, ylim = c(0,80), xlim = c(-2, 8), breaks = 14)
hist(df_outlier$y_no, ylim = c(0,80), xlim = c(-2, 8), breaks = 14)

hist(df$y_yes, ylim = c(0,80), xlim = c(2, 10), breaks = 14)
hist(df_outlier$y_yes, ylim = c(0,80), xlim = c(2, 10), breaks = 14)
