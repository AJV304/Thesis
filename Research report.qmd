...
With this simulation study, I intend to close this gap in the literature and explore the effects of typical preregistration deviations on research outcomes and the quality of published results. Specifically, I aim to answer the question: ‘How do deviations from preregistrations affect Type I and Type II error rates in research?’. The effect of divergences is studied in three domains: sample size, outlier identification, and the statistical model. Different deviations are simulated in each of these domains and compared to a baseline condition with no deviations. This project investigates this question in the context of behavioral psychology and uses linear regression as the main model of interest. The effects are examined across two scenarios: a no-effect scenario, and an effect scenario. The aim of this study is descriptive and therefore no specific hypotheses are formalized.

As a secondary goal, this project also addresses the issue of reasons for deviations being unknown. Each deviation condition is initially simulated as random and the outcomes will be used to assess type I and type II error. Next, deviations will be simulated and compared to the baseline condition, results will be chosen opportunistically from either condition after which error rates will then be compared.

With these simulations I hope to provide clarity on the effect of deviations from preregistration on type I and type II errors, as well as shed more light on the effect of picking deviations opportunistically.

## Methods

### Aims

The aim of this simulation study is to investigate the impact of making changes to analysis plan on the type I and type II error rates.

### Conditions

I identified important deviations to investigate based on (1) how often they occur, (2) their potential impact and (3) their justifiability. These deviations occur in the following domains: sample size, outlier identification, and statistical model. Within these domains, multiple conditions will be simulated and compared to a baseline condition without any deviations.
...
The most common method for excluding outliers in the social sciences is the Z-score. A minimum of three standard deviations is often maintained as the rule of thumb. Within linear regression, it is also common to base outliers on influence. One of the more commonly used methods is Cook's distance. Based on this, the baseline condition will be to not exclude any datapoints, with deviation conditions based on z-score and Cook's distance @tbl-conditions.

<!-- #### Hypotheses -->

<!-- @akkerhypothesis found that 60% of published papers contain a difference in hypotheses when compared to their corresponding preregistration. However, there is no evidence that papers with or without deviations in their hypotheses report more significant results. The deviations that occur are varied and are often due either to improper specification in the preregistration [@heirene_preregistration_2024]. Another reason that researchers might choose to alter their hypotheses during their research is to be able to select hypotheses with desired results [@john]. Common deviations are the adding or removing of a hypothesis in the final paper [@akkerhypothesis], this is commonly done by researchers to present a more convincing argument [@john]. Even though they occur frequently, changes in the hypotheses are ranked as the least justifiable deviations by researchers themselves [@willroth2024]. The following deviations will be simulated. -->
...
<!-- | subgrouping\* <!--- too close to outlier/covariate? --->

<!-- : Hypothesis Deviations {#tbl-hyp} -->

#### Statistical model

Amongst all of the aforementioned domains, the least is known about why people deviate from their preregistered model. Many areas to exist within the statistical model in which researchers can choose to change their predetermined plans. Including, but not limited to, changes in the dependent variable, independent variable, statistical inference criteria and the actual model that is assessed. Different research have investigated deviations in this domain differently. @claesen2021 shows a divergence rate of 70% within the 'analysis' domain, of which most divergences go unreported. Whereas @vandenakker2024a shows inconsistencies in 40% of the statistical models.

As @lakens argues, at times it might be necessary to alter the statistical model used. For example, when a variable has been measured at a different level than expected or when data from two different indicators cannot be combined. However, changing the statistical model can also be done opportunistically. Adding an extra dependent variable can increase the type I error rate to 9.5%, and the addition of a covariate can increase it to 11.7% [@simmons2011]. Therefore, the reasoning behind the change becomes very important. Deviations due to failing to properly specify how variables would be operationalized in the preregistration are therefore also seen as a larger shortcoming than model deviations due to unforeseen consequences [@willroth2024]. Within the statistical model, three deviation conditions are simulated; outcome switching, adding a continuous covariate, and adding a dichotomous covariate (@tbl-conditions.)
...
<!-- | parameter/scenario | $H0 = true$ | $Ha = true$ | -->

<!-- |--------------------|--------------------------|--------------------------| -->

<!-- | Independent variable | $X \sim \mathcal{N}(\mu, \sigma^2)$ | $X \sim \mathcal{N}(\mu, \sigma^2)$ | -->

<!-- | Random error | $\epsilon \sim \mathcal{N}(\mu, \sigma^2)$ | $\epsilon \sim \mathcal{N}(\mu, \sigma^2)$ | -->

<!-- | Regression coefficient | $\beta_1 = 0$ | $\beta_1 = 0.30$ | -->

<!-- | Intercept | $\beta_0 = 0 $ | $\beta_0 = 0$ | -->

<!-- | Continuous demographic variable | $Z \sim \mathcal{N}(\mu, \sigma^2)$ | $Z \sim \mathcal{N}(\mu, \sigma^2)$ | -->

<!-- | Dichotomous demographic variable | $D \sim \text{Bernoulli}(0.5)$ | $D \sim \text{Bernoulli}(0.5)$ | -->

<!-- | Sample size | $150$ | $150$ | -->

<!-- : Data generating conditions {#tbl-DGM}{.striped .hover} -->

### Estimands

Each condition will be assessed on how they affect the type I and type II error. The type I error refers to a false-positive, or rejecting the null-hypothesis when it is true. In this study that would mean detecting an effect in the no-effect scenario. Type I error is generally acceptable at a rate of 5% or below, based on an alpha level of 0.05. In this case, a rate of higher than 5% is considered an inflated type I error rate.
...
| Parameter        | Baseline condition | Deviations conditions    |
|------------------|--------------------|--------------------------|
| Sample size      | 200                | +5                       |
|                  |                    | +30                     |
|                  |                    | -5                      |
|                  |                    | -30                      |
| Outliers         | None               | Based on Z-scores        |
|                  |                    | Based on Cook's distance |
| Covariates       | None               | Continuous covariate     |
...
Performance is assessed through the Type I and Type II error. Furthermore, the regression coefficient and confidence interval are collected as well.
