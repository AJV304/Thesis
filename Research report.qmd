...
The inability to reliably reproduce findings in the field of psychology is a known problem. Only 36% of significant results were found to be reproducible [@opensciencecollaboration2015]. One reason for this "replication crisis" is "researcher degrees of freedom" [@simmons2011]. This term describes the flexibility that researchers have in decisions made during their work. This includes decisions such as how many observations to collect, which statistical model to use and which covariates to add. When these decisions are made during data collection or analysis, researchers can make opportunistic choices to steer their results towards desired outcomes. Such opportunistic choices are called *p*-hacking and can dramatically increase false-positive rates [@stefan2023].

The preregistration is proposed as a solution to limit these types of practices. Templates are available to researchers, to inform them about which information is important to preregister. The preregistration is then uploaded to a public repository with a time stamp of when it was published. Peer reviewers, and others, have access to the preregistration and can evaluate it before the start of the study or compare it to the final research paper. 
...
In phase 1, the type I and type II error rates are compared between the baseline result and deviation result. In phase 2, the effect of choosing whether or not to deviate is examined. This is done by simulating each deviation, but instead of automatically reporting the deviation result, a choice is made. In each iteration the *p*-values of the deviation result and baseline result are compared, the most desired *p*-value is reported as the outcome for the deviation condition. The error rates will then be compared between the baseline results and the best values result. An overview can be found in @fig-phases.

![Condition Comparisons](Visuals/phases.png){#fig-phases}

*Note.* This figures illustrates the study phases. In phase 1, the baseline result is compared to the deviation result. In phase 2, the baseline result is compared to the best result.
